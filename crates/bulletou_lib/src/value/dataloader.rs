...
use crate::{
    ...
    value::{
        fast_batch::FastBatchHost,
        loader::{self, PreparedData},
    },
};
...
pub struct FastValueDataLoader<I, O, D, W>
where
    I: SparseInputType,
    I::RequiredDataType: loader::LoadableDataType,
    O: OutputBuckets<I::RequiredDataType>,
...
impl<I, O, D, W> FastValueDataLoader<I, O, D, W>
where
    I: SparseInputType,
    I::RequiredDataType: loader::LoadableDataType,
    O: OutputBuckets<I::RequiredDataType>,
    ...
    D: loader::DataLoader<I::RequiredDataType>,
{
    pub fn map_fast_batches<F: FnMut(FastBatchHost) -> bool>(
        self,
        batch_size: usize,
        mut f: F,
    ) -> Result<(), DataLoadingError> {
        let FastValueDataLoader { dataloader, steps, threads, wdl } = self;
        let start_batch = steps.batches_per_superbatch * (steps.start_superbatch - 1);
        ...
        dataloader.load_and_map_batches(start_batch, batch_size, |batch| {
            let blend = wdl.blend(batch_no, superbatch, steps.end_superbatch);
            let prepared_data = dataloader.prepare(batch, threads, blend);
            let fast_batch = FastBatchHost::from(prepared_data);

            debug_assert!(fast_batch.validate().is_ok());
