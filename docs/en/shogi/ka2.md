...
`--arch NNUE_ka2_256x2_32_32` trains a YaneuraOu NNUE whose input is `FeatureSet<Features::K, Features::A2>` (architecture file `source/eval/nnue/architectures/ka2_*.h`, auto-generated by `nnue_arch_gen.py`). The network is the same 4-layer ClippedReLU as [HalfKP](halfkp.md) and [K-P](kp.md); only the input feature differs.

The closely-related `--arch SFNN_ka2_1536_15_32_k3k3` trains the same input feature with the SFNN-1536 architecture (LayerStacks=9; see [tutorial: Run the training](../tutorial/3-train.md) and [SFNN-1536](sfnn-1536.md)).
...
Everything else (training schedule flags, save layout, resume from `state.bin`, top-level `summary-learn.csv`) is identical to [HalfKP](halfkp.md) — only `--arch` (and the input dim) differs.
