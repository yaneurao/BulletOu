...
| `--scale` | Eval-to-score sigmoid scale for the sigmoid-loss target. If omitted, BulletOu estimates it from the teacher data | omitted |
| `--scale-calibration-positions` | Number of teacher-prefix positions used to estimate `--scale` | 100000 |
...
For the meaning of the scheduling units, see [Advanced: Adjust training settings](../advanced/tuning.md).
...
`bulletou`'s default is pure eval (`--lambda 1.0`) with the tatara-aligned `--lr 8.75e-4`. For production-quality KPPT, adjust `--lambda` and the learning rate along the above lines.
