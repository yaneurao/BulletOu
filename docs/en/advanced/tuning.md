...
1 iteration:
  ...
  use the two qloss values to move the parameters partway along the SPSA-estimated direction
  candidate: train 8 sb from the base checkpoint with the moved parameters
  accept the candidate only if its qloss is lower than the starting qloss
```

`--sb-per-trial 8` means each trial trains for 8 sb. With `--update-mode spsa`, the runner normally runs probe A/B and then one candidate trial. The probes measure which way to move; the candidate qloss is the value used for the actual accept/retry decision. When accepted, the accepted path advances by 8 sb.
...
With `--use-worker`, the runner starts `bulletou.exe worker` once and keeps one GPU-resident training session open. For probe measurement, the worker snapshots/restores GPU weights and optimizer state, so each trial avoids process startup, CUDA warmup, and trial checkpoint saving. With `--update-mode spsa`, the worker runs the candidate once, keeps it only if the candidate qloss is lower than the starting qloss, and otherwise restores the starting state.
