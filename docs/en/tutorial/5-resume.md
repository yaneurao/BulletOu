...
- The cumulative `summary-learn.log` keeps appending CSV rows for the resumed run. The LR scheduler restarts each run so the superbatch counter resets to 1, but the `positions` column continues from the previous run's max (read off the existing `summary-learn.log` at startup).
