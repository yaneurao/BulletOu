...
| `--threads` | CPU workers for teacher batch preparation. If omitted or set to `0`, BulletOu uses the OS logical thread count (`available_parallelism()`). Explicit values are preserved | auto (= logical threads) |
| `--loader-threads` | CPU workers for HCPE decode. If omitted or set to `0`, BulletOu uses the same auto value as `--threads`. Lower it explicitly if decode workers starve GPU upload/main threads | auto (= logical threads) |
...
| `--test-positions` | Number of positions to use from `--test-teacher`. If omitted, BulletOu validates on all fixed-record positions in the held-out file or directory. Pass this only when you intentionally want a smaller random/sequential subset | all |
| `--test-batch-size` | GPU batch size for `--test-teacher` validation. Larger values reduce validation chunk overhead but use more VRAM; lower it if validation hits cudaMalloc | 65536 |
...
`--loss-pow-exp` and `--wrm-nnue2score` follow tatara's convention. The `--loss-pow-exp` default is `2.0` (squared error in WRM space). Use `2.5` for the commonly reported nnue-pytorch-style setting. The `--wrm-nnue2score` default is `600`.
...
`--loss-pow-exp` and `--wrm-nnue2score` affect WRM loss. A WRM run's `test_value_loss` uses a different formula from the legacy sigmoid-MSE loss, so do not compare the raw loss number directly against an older non-WRM run. Compare runs with the same WRM setting, or use accuracy / engine strength.
...
This does not change the loss formula, so `test_value_loss` is directly comparable with runs that use the same loss setting. Treat it as a separate ON/OFF experiment from the default WRM loss.
