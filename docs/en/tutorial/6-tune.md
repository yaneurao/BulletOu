...
| `--threads` | CPU workers for teacher batch preparation. If omitted or set to `0`, BulletOu uses a throughput-oriented auto value: `logical_threads * 2`, capped at 24. Explicit values are preserved | auto (= min(logical*2, 24)) |
| `--loader-threads` | CPU workers for HCPE decode. If omitted or set to `0`, BulletOu uses the same auto value as `--threads`. Lower it explicitly if decode workers starve GPU upload/main threads | auto (= min(logical*2, 24)) |
