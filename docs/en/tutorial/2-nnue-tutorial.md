...
- **`.pack`** — PackedSfenValue, generated by YaneuraOu's `gensfen` command (yaneurao style)
- **`.hcpe`** — HuffmanCodedPosAndEval, the dlshogi-style fixed-length 38-byte record format

Both are supported; the choice depends on which generator you used (or which shared dataset you have).

Options for obtaining data:

- **Generate your own** with YaneuraOu's `gensfen` (`.pack`) or dlshogi's data generator (`.hcpe`). See each project's documentation; a few hundred million positions is the typical scale, but for this tutorial even 10–100 million is enough.
- **Use a shared dataset**. The shogi community shares both `.pack` and `.hcpe` files on various sites. Make sure the source is trustworthy.
...
- **`shogi_simple`** — reads `.bin` / `.pack` (PackedSfenValue, the yaneurao `gensfen` format)
- **`shogi_simple_hcpe`** — reads `.hcpe` (HuffmanCodedPosAndEval, the dlshogi-style format)
