...
| `--start-wdl` / `--end-wdl` | WDL-lambda linear schedule. **WDL** = Win/Draw/Loss = the game-result label that each teacher position carries alongside the eval (W=1.0, D=0.5, L=0.0, from side-to-move perspective). Loss target is `λ × game_result + (1−λ) × teacher_eval`; `λ` linearly interpolates from `start` (superbatch 1) to `end` (last superbatch) | 0.0 / 0.0 (= pure eval) |
