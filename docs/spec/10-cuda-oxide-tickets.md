...
- Rejected experiments / cautions:
  ...
  - an SFNN fused pairwise/L0 variant that split `l0b` bias-gradient accumulation into a separate row-wise reduction kernel passed tiny GPU correctness smokes but regressed the real bs131k/6-step WRM profile badly: L0 rose from `53.644ms` to `100.640ms`, total backward from `67.928ms` to `114.926ms`, and throughput from `1050585` to `791291` pos/s, so it was reverted;
  - an SFNN fused pairwise/L0 inner-loop refactor that combined row0/row1 factorized virtual-feature atomic handling passed tiny GPU correctness smokes, but did not improve real measurements: the 6-step profile showed L0 `53.891ms` vs the original `53.644ms`, and same-session 4M A/B repeats were noise-level (`pair=1331265/1277590` pos/s, `baseline=1231996/1287913` pos/s), so it was reverted;
