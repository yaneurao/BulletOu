...
SPSA runner は短い trial の qloss 差でハイパーパラメーターを判断する。trial の学習率が高すぎると、ハイパーパラメーター差ではなく短期的な qloss の振動を採用してしまう。基本方針として、SPSA用の trial は通常の追加学習より小さめの `--lr` / `--lr-min` を使う。高lrを使う場合は `--sb-per-trial 16` や `32` に増やす。
