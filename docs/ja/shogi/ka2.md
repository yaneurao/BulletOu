...
KA2の盤上駒は、盤面を1回走査して駒種・色別にまとめてから特徴番号へ変換する。
旧実装の「13駒種×2色ごとに81マスを全走査する」処理は行わない。
特徴番号・個数だけでなく、駒種→色→マスの列挙順も維持しているため、
FTでの加算順序を変えずにCPUの準備処理を減らせる。追加オプションやGPUメモリは不要。
既存の設定・checkpoint・`nn.bin`形式は変更せずに使用できる。

小さいネットワークでも、教師データの読み込み・CPUでの特徴抽出が律速になると
学習速度は上がらない。`--cuda-cpp-diagnostics-rate 1` を一時的に指定すると、
出力先の `cuda-cpp-diagnostics.log` で各sbのデータ準備時間・batch待ち時間を確認できる。
診断時はCUDAの同期が入るので、通常の学習では既定の無効設定に戻す。

### 主要 CLI フラグ

| フラグ | 意味 | デフォルト |
|---|---|---|
| `--arch` | `NNUE_ka2_<L1>x2_<L2>_<L3>` or `SFNN_ka2_<FT>_<H1>_<H2>[_cN_sMxG]_<stack>` | (required; target is inferred) |
