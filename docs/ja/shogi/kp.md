...
L1 / L2 / L3 サイズは `--arch` で選ぶ。preset リストは NNUE_HALFKP と共通 (`256x2-32-32` デフォルト、加えて `384x2-8-96` / `512x2-8-64` / `768x2-16-64` / `1024x2-8-32` / `1024x2-8-64`)。やねうら王が実エンジンとして配布している `NNUE_kp_*` バイナリは現状 `256x2-32-32` のみだが、学習側は他 preset でも生成できる (実験用)。以下はデフォルト構成の図:
