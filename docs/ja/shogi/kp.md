...
L1 / L2 / L3 サイズは `--arch <L1>x2-<L2>-<L3>` で **自由形式**で指定する (NNUE_HALFKP と同じよく使われるサイズが共通、詳細は [§4.3](../tutorial/4-train.md#43---arch-を指定する))。やねうら王が実エンジンとして配布している `NNUE_kp_*` バイナリは現状 `256x2-32-32` のみだが、学習側は他サイズでも生成できる (実験用)。以下はデフォルト構成の図:
...
| `--arch` | 自由形式 `<L1>x2-<L2>-<L3>` (`L1` は 32 の倍数)。共通サイズ: `256x2-32-32`、`384x2-8-96`、`512x2-8-64`、`768x2-16-64`、`1024x2-8-32`、`1024x2-8-64` | `256x2-32-32` |
