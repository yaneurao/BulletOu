...
L1 / L2 / L3 サイズは `--arch NNUE_kp_<L1>x2_<L2>_<L3>` で指定する (NNUE_HALFKP と同じよく使われるサイズが共通。基本のコマンド形は [チュートリアル: 学習を走らせる](../tutorial/3-train.md) を参照)。やねうら王が実エンジンとして配布している `NNUE_kp_*` バイナリは現状 `256x2-32-32` のみだが、学習側は他サイズでも生成できる (実験用)。CLI では `NNUE_kp_256x2_32_32` のように完全名で書く。以下はデフォルト構成の図:
