...
## 2. `--scale 203` だけでは何が起きるか

教師評価値が `scale=600` で作られているのに、BulletOuで `--scale 203` として学習すると、教師の勝率ラベルが尖ります。

例として、教師評価値 `+600` を考えます。
...
元の教師は「勝率73.1%」のつもりで `+600` と書いているのに、`--scale 203` で読むと「勝率95.0%」として扱ってしまいます。

これが「教師データの勝率を歪める」という意味です。

## 3. では、なぜ `--fv-scale` が必要なのか

一方で、NNUE/SFNN は層が浅いため、network output の範囲をある程度大きく取ったほうがよいことがあります。やねうら王側で `FV_SCALE=40` として使いたい場合、学習後の `nn.bin` は次の関係を満たしてほしいです。
...
つまり、評価値 `+600` を出したいなら、network output はだいたい `+600 / 203.2 = +2.95` になります。
