...
`--teacher` だけは例外で、同じ設定のまま教師だけ変える追加学習は自動再開できます。教師が変わったことを検出し、新しい教師ファイルの先頭から読みます ([教師を変えて追加学習する](#4-教師を変えて追加学習する))。
