...
このページでは、保存済みの `nn.bin` を直接調べる2つのコマンドを説明します。
...
`--fv-scale` を省略した場合、BulletOu は `FV_SCALE=40` として計測します。これは学習時のデフォルト設定、つまり教師target用の `--scale 600` と NNUE/SFNN 出力レンジ用の `--fv-scale 40` に合わせた値です。
