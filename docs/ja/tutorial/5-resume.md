...
- `summary-learn.log` (累積版) には新 run の CSV 行がそのまま追記される。superbatch カウンタは 1 から再開するが、`positions` 列は累積される (新 run 開始時に既存 `summary-learn.log` の最大 positions を読み取って続きから書く)。LR は schedule 依存で、`step_gamma` は累積局面数に基づいて継続、`step` / `cos` は新 run の cycle として再開する
