...
`--buffer-mb` は読み込みバッファのサイズであり、シャッフル用の指定ではない。BulletOu は、`--teacher-shuffle-buffer-batches` を省略すると学習時 shuffle をデフォルトで有効にする。このとき `batch_size × batches_per_superbatch × superbatches` 局面、つまり 1 epoch 分の CPU window を 2 個確保する。片方から mini-batch を取り出している間に、もう片方を読み込み・Fisher-Yates shuffle する。小さい window を明示したい場合は `--teacher-shuffle-buffer-batches N`、学習時 shuffle を無効化したい場合は `--teacher-shuffle-buffer-batches 0` を指定する。
...
- **学習時 shuffle**: デフォルトで有効。たとえば `batch_size=65536`, `positions-per-superbatch=40000000`, `superbatches=36` なら実効 `batches_per_superbatch=610` なので、デフォルト window は `610 × 36 = 21960` batches になる。40 byte の PSV/HCPE 互換 record として 1 window は約 53.6 GiB、double buffer なので合計約 107.2 GiB の CPU メモリを使う。実際の確保量は学習開始時に表示される。小さくしたい場合は `--teacher-shuffle-buffer-batches 61` のように明示する。
