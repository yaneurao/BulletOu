...
| `--sfnn-factorized-l1` / `--no-sfnn-factorized-l1` | SFNN state に stack 共有 factorizer tensor（dense L1 で使う `l1fw/l1fb`、および `l2fw/l2fb`・`l3fw/l3fb`）が含まれるかが変わる。新規 SFNN run では L2/L3 共有項がデフォルトON。古い非factorized SFNN runを再開するときだけ `--no-sfnn-factorized-l1` を使う |
