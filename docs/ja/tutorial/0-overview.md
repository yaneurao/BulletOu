...
| `SFNN_halfkahm2_1536_15_32_k3k3` | やねうら王 NNUEwoSQPT1536 ビルド用の SFNN 評価関数 (HalfKA_hm2 入力)。局面ごとに後段ネットワークを切り替える。詳細は [SFNN-1536 学習リファレンス](../shogi/sfnn-1536.md)、使い方は [§9 LayerStack](9-layerstack.md) | `nn.bin` |
| `SFNN_halfkahm1_1536_15_32_k3k3` | ↑ の HalfKA_hm1 (v1) を使ったアブレーション用 | `nn.bin` |
| `SFNN_ka2_1536_15_32_k3k3` | SFNN-1536 の層構成はそのままで、入力を `K + A2` (1791 次元) に置き換えた版。軽量な比較実験用。 | `nn.bin` |
...
| `KPP_KKPT` | KPPT の KPP 部分を小さくした形式 (KPP のみ手番チャンネルなしでサイズ半減) | 同上 (KPP の配置だけ違う) |
