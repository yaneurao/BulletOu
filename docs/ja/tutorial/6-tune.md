...
| `--teacher-shuffle-buffer-batches` | 学習時 teacher shuffle window。`batch_size × N` 局面のCPU windowを2個確保し、片方を消費中に片方を読み込み・Fisher-Yates shuffleする。省略時の `N` は 1 epoch 分 (`batches_per_superbatch × superbatches`)。`0` で無効 | 1 epoch |
