    ...
    for epoch in 1..=max_epochs {
        ...
        let on_checkpoint_saved = move |superbatch: usize| {
            ...
            match save_yaneuraou_kppt(&ckpt_dir, yaneuraou_scale) {
                Ok(()) => {
                    eprintln!("  also wrote KK_synthesized.bin / KKP_synthesized.bin in {}", ckpt_dir.display())
                }
                Err(e) => {
                    eprintln!("  WARN: failed to write YaneuraOu KPPT binaries in {}: {e}", ckpt_dir.display())
                }
            }
        };
