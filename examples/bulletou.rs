    ...
    I: SparseInputType<RequiredDataType = bulletou_lib::shogi::PackedSfenValue> + Copy,
{
    ...
    eprintln!(
        "note: nn.bin in each save dir targets a YaneuraOu build with edition\n  \
         ...
         Build it with `make normal YANEURAOU_EDITION=<edition>` (the architecture\n\
         header is auto-generated by nnue_arch_gen.py for unknown editions). Then\n\
         load via `setoption name EvalDir value <output>/000N` and verify with\n\
         `isready`+`bench`. If the engine was built for a different arch, an\n\
         `info string Warning: NNUE hash mismatch` line will appear (engine still\n\
         loads); for a matching edition this should not appear."
    );
}
