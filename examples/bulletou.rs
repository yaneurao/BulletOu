...
impl std::str::FromStr for NnueArch {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        if tokens.len() < 5 {
            return Err(format!(
                "invalid arch `{s}`: expected `NNUE_<feature>_<L1>x2_<L2>_<L3>` or `SFNN_<feature>_<FT>_<H1>_<H2>[_gN|_cN_sMxG]_<k3k3|k9k9|hand64|hand64_k3k3|hand64_k9k9|hand256|hand256_k3k3|hand256_k9k9|hand1024|hand1024_k3k3|hand1024_k9k9>`"
            ));
        }
        ...
        match family {
            ...
            NnueArchFamily::Sfnn => {
                if tokens.len() < 6 {
                    return Err(format!(
                        "invalid arch `{s}`: expected `SFNN_<feature>_<FT>_<H1>_<H2>[_gN|_cN_sMxG]_<k3k3|k9k9|hand64|hand64_k3k3|hand64_k9k9|hand256|hand256_k3k3|hand256_k9k9|hand1024|hand1024_k3k3|hand1024_k9k9>`"
                    ));
                }
                ...
                let layerstack = match tokens[layerstack_start..].join("_").to_ascii_lowercase().as_str() {
                    ...
                    "ls9" => {
                        return Err(format!(
                            "invalid arch `{s}`: ls9 is no longer supported; use k3k3, k9k9, hand64, hand64_k3k3, hand64_k9k9, hand256, hand256_k3k3, hand256_k9k9, hand1024, hand1024_k3k3, or hand1024_k9k9"
                        ));
                    }
                    other => {
                        return Err(format!(
                            "invalid arch `{s}`: unsupported SFNN layer stack `{other}`; expected k3k3, king3_by_king3, k9k9, king9_by_king9, hand64, hand64_k3k3, hand64_king3_by_king3, hand64_k9k9, hand64_king9_by_king9, hand256, hand256_k3k3, hand256_king3_by_king3, hand256_k9k9, hand256_king9_by_king9, hand1024, hand1024_k3k3, hand1024_king3_by_king3, hand1024_k9k9, or hand1024_king9_by_king9"
                        ));
                    }
                };
...
fn effective_sfnn_factorized_stack(args: &Args) -> bool {
    !args.no_sfnn_factorized && args.resolved_eval_type().is_some_and(EvalType::uses_layerstack)
}
...
struct Args {
    ...
    #[arg(long = "sfnn-factorized", alias = "sfnn-factorized-l1", conflicts_with = "no_sfnn_factorized")]
...
fn write_cuda_cpp_sfnn_numbered_checkpoint(
    ...
    std::fs::create_dir_all(&dir).map_err(|err| format!("failed to create {}: {err}", dir.display()))?;
    write_cuda_cpp_sfnn_nn_bin(&dir.join("nn.bin"), feature_kind, shape, weights)?;
...
fn load_cuda_cpp_sfnn_initial_state(
    ...
    for (wants, has, label, resume_hint) in [
        (
            ...
            "Pass --no-sfnn-factorized to resume/use this state unchanged, or start a new experiment.",
        ),
        (
            wants_l2_l3f,
        ...
        match (wants, has) {
            ...
            (false, true) => {
                return Err(format!(
                    "loaded SFNN state {} contains {label}, but --no-sfnn-factorized disabled shared stack terms. \
                     Remove --no-sfnn-factorized to resume this state.",
                    path.display()
                ));
            }
            _ => {}
        ...
        for out_col in 0..l1_out {
            for in_col in 0..l1_pad_in {
                let q = if in_col < shape.ft_size {
                    let mut value = l1w_for_export[stack * l1_out * shape.ft_size + out_col * shape.ft_size + in_col];
