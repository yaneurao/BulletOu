    ...
    fn parse(raw_args: impl Iterator<Item = String>) -> bulletou_cuda_oxide_runtime::Result<Self> {
        ...
        while let Some(arg) = args.next() {
            match arg.as_str() {
                ...
                "--scale" | "--sigmoid-scale" => {
                    parsed.sigmoid_scale = parse_f32_arg(required_arg(&mut args, "--scale")?, "--scale")?;
                }
...
fn run_nnue_loss_ranger_step_smoke(args: Args) -> bulletou_cuda_oxide_runtime::Result<()> {
    ...
    for (step_idx, train_case) in train_batches.iter().enumerate() {
        ...
        let cpu_loss_trace = cpu_loss_case.cpu_loss_trace(args.loss_kind, args.sigmoid_output_scale);
...
fn run_loss_smoke(args: Args) -> bulletou_cuda_oxide_runtime::Result<()> {
    ...
    let cpu_trace = case.cpu_loss_trace(args.loss_kind, args.sigmoid_output_scale);
    let ptx = match args.ptx {
        Some(ptx) => ptx,
        None => default_nnue_ptx()?,
    };
    ...
    match args.loss_kind {
        LossKind::SigmoidMse => loss_forward::launch_sigmoid_mse_loss_from_buffers(
            &stream,
            &module,
            &device_batch.outputs,
            &device_batch.targets,
            &device_batch.entry_weights,
            args.sigmoid_output_scale,
            &mut workspace,
        )?,
...
fn run_sfnn_output_backward_smoke(args: Args) -> bulletou_cuda_oxide_runtime::Result<()> {
    ...
    if let (Some(cpu_l1fw), Some(cpu_l1fb)) = (&cpu_trace.l1fw_gradients, &cpu_trace.l1fb_gradients) {
        let gpu_l1fw_gradients = backward_workspace.l1fw_gradients.to_host_vec(&stream)?;
        let gpu_l1fb_gradients = backward_workspace.l1fb_gradients.to_host_vec(&stream)?;
        comparisons.push(compare_slices("l1fw_grad", cpu_l1fw, &gpu_l1fw_gradients, args.tolerance)?);
        comparisons.push(compare_slices("l1fb_grad", cpu_l1fb, &gpu_l1fb_gradients, args.tolerance)?);
    }
...
fn run_sfnn_ranger_step_smoke(args: Args) -> bulletou_cuda_oxide_runtime::Result<()> {
    ...
    compare_group!(l1w, &case.l1w, &cpu_trace.l1w_gradients);
    compare_group!(l1b, &case.l1b, &cpu_trace.l1b_gradients);
    macro_rules! compare_optional_group {
        ($name:literal, $weights:expr, $gradients:expr, $device_weights:expr, $state:expr) => {{
            let expected = cpu_single_ranger_update_trace($weights, $gradients, params)?;
            let gpu_weights = $device_weights.to_host_vec(&stream)?;
...
fn usage() -> &'static str {
    "Usage:\n\
       ...
       bulletou-cuda-train --loss-smoke [--loss-kind sigmoid-mse|wrm] [--loss-case tiny|weighted] [--model-output-scale <F32>] [--ptx <PATH>] [--device <ID>] [--tolerance <F32>] [--debug-readback]\n\
       ...
       bulletou-cuda-train --sfnn-dense-backward-smoke [--sfnn-forward-case tiny|factorized-tiny|halfka2] [--sfnn-forward-fixture <PATH>] [--ptx <PATH>] [--device <ID>] [--tolerance <F32>]\n\
...
fn validate_nnue_teacher_schedule_args(args: &Args) -> bulletou_cuda_oxide_runtime::Result<()> {
    ...
    if !(args.sigmoid_scale.is_finite() && args.sigmoid_scale > 0.0) {
        return Err(bulletou_cuda_oxide_runtime::Error::Smoke("--scale must be finite and > 0".to_string()));
    }
    if !(args.sigmoid_output_scale.is_finite() && args.sigmoid_output_scale > 0.0) {
        return Err(bulletou_cuda_oxide_runtime::Error::Smoke(
            "--model-output-scale must be finite and > 0".to_string(),
        ));
    }
...
struct SfnnOutputBackwardTrace {
    ...
    l1w_gradients: Vec<f32>,
    l1b_gradients: Vec<f32>,
    l1fw_gradients: Option<Vec<f32>>,
    l1fb_gradients: Option<Vec<f32>>,
    stm_l0_gradients: Vec<f32>,
    nstm_l0_gradients: Vec<f32>,
...
impl LossSmokeCase {
    ...
    fn cpu_loss_trace(&self, kind: LossKind, sigmoid_output_scale: f32) -> LossSmokeTrace {
        ...
        for ((&output, &target), &entry_weight) in self.outputs.iter().zip(&self.targets).zip(&self.entry_weights) {
            let (loss, output_gradient) = loss_value_and_gradient(kind, output, target, sigmoid_output_scale);
...
impl SfnnForwardCase {
    ...
    fn cpu_output_backward_trace(&self, forward: &SfnnForwardTrace) -> SfnnOutputBackwardTrace {
        ...
        let (mut combined_gradients, l1w_gradients, l1b_gradients) = sfnn_stacked_affine_backward_trace(
            &forward.combined,
            &l1_gradients,
            &self.l1w,
            &self.buckets,
        ...
        let (l1fw_gradients, l1fb_gradients) = match (&self.l1fw, &self.l1fb) {
            (Some(l1fw), Some(_l1fb)) => {
                let (weight_gradients, bias_gradients) = sfnn_shared_l1_backward_trace(
                    &forward.combined,
                    &l1_gradients,
                    l1fw,
                ...
                (Some(weight_gradients), Some(bias_gradients))
            }
            _ => (None, None),
        };
...
#[cfg(feature = "cuda")]
fn sfnn_shared_l1_backward_trace(
    inputs: &[f32],
    output_gradients: &[f32],
    weights: &[f32],
    input_gradients: &mut [f32],
    batch_size: usize,
    input_dim: usize,
    output_dim: usize,
) -> (Vec<f32>, Vec<f32>) {
    ...
    for sample in 0..batch_size {
        ...
        for out_col in 0..output_dim {
            ...
            for in_col in 0..input_dim {
                let input_value = inputs[sample_input_start + in_col];
                let weight_idx = in_col * output_dim + out_col;
                input_gradients[sample_input_start + in_col] += grad * weights[weight_idx];
                weight_gradients[weight_idx] += grad * input_value;
            }
        }
...
#[cfg(feature = "cuda")]
fn loss_value_and_gradient(kind: LossKind, output: f32, target: f32, sigmoid_output_scale: f32) -> (f32, f32) {
    match kind {
        LossKind::SigmoidMse => {
            let output_inv_scale = if sigmoid_output_scale.is_finite() && sigmoid_output_scale > 0.0 {
                1.0 / sigmoid_output_scale
            } else {
                1.0
            };
            let prediction = sigmoid(output * output_inv_scale);
            let error = prediction - target;
            let loss = error * error;
            let gradient = 2.0 * error * prediction * (1.0 - prediction) * output_inv_scale;
            (loss, gradient)
        }
