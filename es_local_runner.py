...
def load_parameters(path: Path) -> tuple[dict[str, Any], dict[str, ParameterSpec], EsSettings, RunSettings]:
    ...
    if candidate_validation_rate < 0:
        raise ValueError("es.candidate_validation_rate must be >= 0; 0 disables f32 candidate validation")
    if candidate_quantized_validation_rate < 0:
        raise ValueError(
            "es.candidate_quantized_validation_rate must be >= 0; 0 disables quantized candidate validation"
        )
    ...
    if metric_needs_validation and candidate_validation_rate == 0:
        raise ValueError(
            f"es.metric {metric!r} requires es.candidate_validation_rate > 0; "
            "use a large value such as 9999 for final-stage-only validation"
        )
    if metric_needs_quantized_validation and candidate_quantized_validation_rate == 0:
        raise ValueError(
            f"es.metric {metric!r} requires es.candidate_quantized_validation_rate > 0; "
            "use a large value such as 9999 for final-stage-only quantized validation"
        )
...
def build_train_args(
    ...
    *,
    ...
    validation_rate = 0 if settings.candidate_validation_rate == 0 else max(
        1, min(settings.candidate_validation_rate, stage_delta_sbs)
    )
    quantized_validation_rate = 0 if settings.candidate_quantized_validation_rate == 0 else max(
        1, min(settings.candidate_quantized_validation_rate, stage_delta_sbs)
    )
    cmd = [
        "--settings-file",
